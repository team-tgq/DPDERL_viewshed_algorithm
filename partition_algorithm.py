...
class PartitionAlgorithm:
    def __init__(self, horizontal_start_angle, horizontal_end_angle,
                 ...
                 is_observe_not_in_left_top_diagonal, is_observe_not_in_right_bottom_diagonal, result, quadrant, is_in_grid_point):
    ...
    def analysis_bottom(self):
        ...
        # test_line = start_base_line
        # test_d = []
        # all_d = []
        # while test_line is not None:
        #     test_d.append(test_line.end_d)
        #     test_line = test_line.next
        # all_d.append(test_d)
        y_grid_index -= 1
        y_distance_index -= 1
        # 直线线性变换,y均匀增加,x增加也为固定值.(需要根据角度来对扩展边界值进行优化)
        # const_increase_x = 0 if east_k == 0 or east_k == math.inf else self.adjust_bound(
        #     self.dem.dy / (east_k * self.dem.dx))
        # const_decrease_x = 0 if west_k == 0 or west_k == math.inf else self.adjust_bound(
        #     self.dem.dy / (west_k * self.dem.dx))
        # 计算出增加或者减少个数  去拓展边界
    ...
    def analysis_left(self):
        ...
        # const_increase_y = 0 if north_k == math.inf else self.adjust_bound(north_k * self.dem.dx / self.dem.dy)
        # const_decrease_y = 0 if south_k == math.inf else self.adjust_bound(south_k * self.dem.dx / self.dem.dy)
        x_grid_index -= 1
        x_distance_index -= 1
    ...
    def analysis_left_right(self):
        ...
        while x_grid_index <= self.max_x and start_distance_y_index < 2 * self.y_grid_count - 1:
            # 防止超出结果数组边界,回调索引
            if start_distance_y_index < 0:
                start_y_index += abs(const_increase_y)
                start_distance_y_index += abs(const_increase_y)
    ...
    def analysis_right_bottom(self):
        ...
        while y_grid_index >= self.min_y and start_distance_x_index < 2 * self.x_grid_count - 1:
            if start_distance_x_index < 0:
                start_distance_x_index += abs(const_decrease_x)
                start_x_index += abs(const_decrease_x)
            x_grid_index = start_x_index
            x_distance_index = start_distance_x_index
